...
//		log.debug("Request to save Accounts : {}", accountDTO);
		...
		Resource accountManager = resourceRepository.getOne(accountManagerCode);
		
		City location = locationRepository.loadCity(cityId);
		
		Account account= accountRepository.save(Account.ofCustomer(customerName).reportingTo(customerReportingManager).
